<template>
  ...
  <admin-layout>
    ...
    <datatables
      ref="datatables"
      class="pt-4"
      :route-uri="route('admin.surat.pengajuan.table')"
      ...
      <template #after.reload-button>
        <Link v-if="hasAccess('module.surat.pengajuan.index', $page.props.currentUser.jabatan.hak_akses)" :href="route('admin.surat.pengajuan.create')" class="p-3 text-center shadow-md rounded-md font-semibold text-white bg-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-300">
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      function deleteRow() {
        form.delete(route('admin.surat.pengajuan.destroy', { pengajuan: form.value.slug }), {
          onSuccess: () => {
            form.reset()
